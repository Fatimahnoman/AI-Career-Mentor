...
export function ResumeAnalysisResult({ data, originalFile }: { data: AnalysisData; originalFile: File }) {
...
${suggestion}`)
  ...
  const copyToClipboard = () => {
    const text = `Summary: ${data.summary}\n\nStrengths: ${data.strengths.join(", ")}\n\nSuggestions: ${data.careerSuggestions.join("\n")}`
      ...
      <div className="glass rounded-3xl p-8 border border-primary/20">
        <div className="flex justify-between items-center mb-4">
          ...
          <button 
            ...
            className="flex items-center gap-2 bg-primary text-white px-5 py-2.5 rounded-xl font-bold text-sm hover:scale-105 transition-transform"
          >
            {downloading ? "Generating..." : <><Download className="h-4 w-4" /> Download Final PDF</>}
          </button>
        </div>
        <textarea 
          value={editedText}
        ...
        <div className="md:col-span-2 rounded-3xl glass p-8 bg-linear-to-br from-purple-500/5 to-transparent border-purple-500/10">
          <div className="flex justify-between items-center mb-8">
            ...
            <button 
              ...
              className="flex items-center gap-2 bg-purple-100 text-purple-700 px-5 py-2.5 rounded-xl font-bold text-sm hover:scale-105 transition-transform"
            >
